...
let jsonGen = () => {
  ...
  (fs.readdirSync("programs")).forEach((element) => {
    let files = fs.readdirSync(`programs/${element}`);
    let supports = [];
    files.forEach((file) => {
      if (file.slice(0, 4) == "main") {
        supports.push(file.slice(5));
      }
    });
    let obj = {
      "prog-name": element,
